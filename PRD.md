...
Scheduled run fires
  ...
  → scraper reports errorType: INVALID_PASSWORD | CHANGE_PASSWORD | ACCOUNT_BLOCKED | TIMEOUT | GENERIC
...
Scraper run
  └── produces Transaction[]
        └── filtered by: zero-amount check, future-date check, state.db dedup check
