...
// Prune fingerprints older than this many days to keep the file bounded
const FINGERPRINT_PRUNE_DAYS = 180;
...
function seenPath(): string {
  ...
  return resolve(dir, 'seen-fingerprints.json');
}

type StateFile = Record<string, string>; // companyId → ISO date string
type SeenFile = Record<string, string[]>; // companyId → fingerprint[]
...
function readSeen(): SeenFile {
  ...
  try {
    ...
    logger.warn('Corrupt seen-fingerprints.json — starting fresh');
    return {};
  }
}
...
export function getSeenFingerprints(companyId: string): Set<string> {
  const seen = readSeen();
  return new Set(seen[companyId] ?? []);
}

export function addSeenFingerprints(companyId: string, fingerprints: string[]): void {
  if (fingerprints.length === 0) return;
  const seen = readSeen();
  const merged = new Set([...(seen[companyId] ?? []), ...fingerprints]);

  // Prune entries older than FINGERPRINT_PRUNE_DAYS
  // Fingerprint format: DD/MM/YYYY|amount|name
  const cutoff = new Date();
  cutoff.setDate(cutoff.getDate() - FINGERPRINT_PRUNE_DAYS);
  cutoff.setHours(0, 0, 0, 0);

  const pruned = [...merged].filter((fp) => {
    const datePart = fp.split('|')[0];
  ...
  seen[companyId] = pruned;
  writeSeen(seen);
  logger.debug(
    `${companyId}: saved ${fingerprints.length} new fingerprint(s), ${pruned.length} total after pruning`,
  );
}
